...
# function to loop through each coint and count them
def coin_count(user_money):
    coin_list = []
    coin = []
    for i in range(len(coins)):
        coin = user_money // coins[i][1]
        leftovers = user_money % coins[i][1]
        coin_list.append((coin, coins[i][0]))
        user_money = leftovers
    return coin_list

#get user input and convert to pennies
user_money = float(input('How much money do you have, written as a dollar amount (e.g. 10.45)? >'))
user_money = int(user_money * 100)
